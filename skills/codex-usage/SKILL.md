...
The plugin's trusted `UserPromptSubmit` hook may add developer context saying a Codex Usage card is due. That message is deliberately terse, because it is re-read on every later turn; the behaviour is defined here. When it appears, call `show_usage_card` exactly once before other work with the supplied `thread_id`, `live: true`, and `automatic: true`, then continue the user's request without narrating the card call or adding a usage summary of your own. If the tool is unavailable, continue without retrying it. The hook rate-limits this per task (five minutes by default), so do not second-guess or duplicate it. If the hook does not say a card is due, do not render one after an unrelated reply.
