...
If a host omits the card's component metadata, a small reference recovers the original compact snapshot from the task's local MCP process. If that process has restarted and the snapshot is gone, the card shows a retryable loading error rather than presenting newer values as its original snapshot.
